...
def insert_article(topic: str, article: dict):
        ...
        data_object={
            "data": {
                ...
                "teaser": (
                    article.get("teaser", "")
                    if isinstance(article, dict)
                    else getattr(article, "teaser", "")
                ),
                "content": str(
                    article.get("content", "")
                    if isinstance(article, dict)
                    else getattr(article, "content", "")
                )[-32000:],
    ...
    try:
        ...
        print(
            f" -> Inserted into Vector Search: {article.get('title', '') if isinstance(article, dict) else getattr(article, 'title', '')}"
        )
...
def search_archive(query: str, top_k: int = 5) -> list[dict]:
    ...
    try:
        batch_results = data_search_client.batch_search_data_objects(
